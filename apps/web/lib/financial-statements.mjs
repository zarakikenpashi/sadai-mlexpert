function official(entries) {
  return entries.filter((entry) => entry.status === 'validated' || entry.status === 'reversed');
}

function withinPeriod(entry, start, end) {
  if (start && entry.date < start) return false;
  if (end && entry.date > end) return false;
  return true;
}

export function filterStateEntries({ entries, periodStart, periodEnd, journal, accountClass }) {
...
export function buildGeneralBalance({ entries }) {
  ...
  const totals = rows.reduce((acc, row) => ({ debit: acc.debit + row.debit, credit: acc.credit + row.credit, difference: acc.difference + row.balance }), { debit: 0, credit: 0, difference: 0 });
  return { rows, totals };
}

export function buildGeneralLedger({ entries }) {
  const ledger = {};
  for (const entry of official(entries).sort((a, b) => a.date.localeCompare(b.date))) {
    for (const line of entry.lines) {
      if (!ledger[line.account]) ledger[line.account] = { account: line.account, label: line.label, movements: [], closingBalance: 0 };
...
export function buildExportMetadata(metadata) {
  return { ...metadata };
}

export function canExportStates({ permissions }) {
  return permissions.includes('state:read') && permissions.includes('export:create');
}

export function generateStateExportPayloads({ entries, metadata }) {
